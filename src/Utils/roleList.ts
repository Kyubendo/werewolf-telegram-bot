...
export const roles: { [k: string]: () => Role } =
    {
        ...
        //     dayAction: 'Выбирает игрока. ' +
        //         'Перед голосованием бросает в него снежком, тем самым замораживая его на сутки.',
        //     notes: [
        //         'Все игроки узнают, в кого Снеговик бросил снежок.',
        //         'Если волк съест Снеговика, то все волки будут заморожены на одну ночь.',
        //         'Снеговик может бросить всего три снежка. После третьего броска у него закончится снег и он умрёт.'
        //     ]
        // }),
        wild_child: () => ({
            ...roles.villager(),
        ...
        puppet_master: () => ({
            ...
            team: 'Сам в команде',
            weight: '-6',
            winCondition: 'остаётся один на один с другим игроком или остаётся единственным выжившим.',
            notes: [
                'Кукловод не может заставить подконтрльного игрока выбрать самого себя, ' +
                'но может заставить волка съесть другого волка.',
                'Если Кукловод выбрал для контроля волка, то он будет управлять всей стаей волков. ' +
                'В таком случае стаю поведёт именно тот игрок, которого выбрал Кукловод.',
                'Если Поджигатель решил сжечь кого-то, то Кукловод не сможет ему помешать.',
                'Если Кукловод выбрал для контроля Купидона, то ему предоставится два выбора.',
            ]
        })
    }
